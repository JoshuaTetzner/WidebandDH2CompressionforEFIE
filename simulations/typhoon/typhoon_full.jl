...
filename = "typhoon"
meshpath = "/home/jt286/Documents/Geometries/typhoon/$(filename)_176709_gmsh.msh"
...
println("\tConstructed scalar potential")
e = assemble((n × Maxwell3D.planewave(; direction=ŷ, polarization=ẑ, wavenumber=k)) × n, X)
...
jldsave("typhoon_$(filename).jld2"; u)
##
...
vtk_grid(
    joinpath(@__DIR__, "../..", "results", "solve_$(filename)_$(numfunctions(X)).vtu"),
    vertexarray(Γ)',
    cellV,
) do vtk
...
u = load("/home/jt286/Documents/WidebandNCA/typhoon_$filename.jld2")["u"]

E0 = 1.0

nphi = 200
phis = 2π .* (0:(nphi - 1)) ./ nphi
rhat = [SVector{3,Float64}(cos(φ), sin(φ), 0.0) for φ in phis]
ff = Maxwell3D.farfield(; wavenumber=k)
Efar = potential(ff, rhat, u, X; threading=BEAST.Threading{:multi})
sigma = 4π .* [sum(abs2, E) for E in Efar] ./ abs2(E0)
sigma_dBsm = 10 .* log10.(max.(sigma, eps(Float64)))

# Close the curve explicitly.
phis_plot = [phis; first(phis)]
sigma_dBsm_plot = [sigma_dBsm; first(sigma_dBsm)]
data = hcat(rad2deg.(phis_plot), sigma_dBsm_plot)

path = joinpath(@__DIR__, "../..", "results", "rcs_xy_polar.csv")
open(path, "w") do io
    println(io, "phi_deg,sigma_dBsm")
    writedlm(io, data, ',')
end
