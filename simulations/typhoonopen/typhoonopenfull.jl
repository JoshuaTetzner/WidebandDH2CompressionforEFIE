...
include(joinpath(@__DIR__, "../..", "src", "utils2.jl"))

## -----------------------------------------------------------------------------
filename = "typhoonrefinedx2"
meshpath = "/home/jt286/Documents/Geometries/typhoon/$(filename).msh"
...
TAxrxr = assembleh2(𝓣Ã, Xr, Xr; k=abs(k̃))#; threading=BEAST.Threading{:cellcoloring})
...
jldsave(joinpath(@__DIR__, "../..", "results", "opentyphoon_$(numfunctions(X)).jld2"); u)
##
...
vtk_grid(
    joinpath(@__DIR__, "../..", "results", "opentyphoon_$(numfunctions(X))"),
    vertexarray(Γ)',
    cellV,
) do vtk
