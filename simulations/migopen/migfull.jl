...
## -----------------------------------------------------------------------------
filename = "mig15open_h0.025"
meshpath = "/home/jt286/Documents/Geometries/mig_open/$(filename).msh"
Γ = CompScienceMeshes.read_gmsh_mesh(meshpath)

# Open geometry: it has a free boundary (Euler characteristic ≠ 2).
@assert numcells(boundary(Γ)) > 0
## -----------------------------------------------------------------------------
...
# Drop free-boundary edges: on an open PEC surface no current flows off the
# rim, so the primal RT space uses interior edges only. This keeps it dual to
# the Buffa-Christiansen space (Buffa-Christiansen functions live on interior
# edges), so the mixed Gram matrix Nyx stays square and invertible.
...
println("\tConstructed scalar potential")
e = assemble((n × Maxwell3D.planewave(; direction=x̂, polarization=ẑ, wavenumber=k)) × n, X)
...
jldsave(joinpath(@__DIR__, "../..", "results", "migopen_$(numfunctions(X))_40h.jld2"); u)
##
...
vtk_grid(
    joinpath(@__DIR__, "../..", "results", "migopen_$(numfunctions(X))_40h"),
    vertexarray(Γ)',
    cellV,
) do vtk
